...
TEST(PhaseA2, FooCtorDtor){
  ...
  EXPECT_EQ(21, Foo::destructions); // 21 for B
  EXPECT_EQ(0, Foo::copies); // 0 for B
}
...
TEST(PhaseA2, ReallocCopy){
  ...
  EXPECT_EQ(2, Foo::destructions); // 2 for B
  EXPECT_EQ(1, Foo::copies); // 1 for B
}
...
TEST(PhaseA3, FooUnderFoo){
  ...
  EXPECT_EQ(1, Foo::constructions);
  EXPECT_EQ(4, Foo::destructions); // 7 for A
  EXPECT_EQ(3, Foo::copies); // 6 for A
...
TEST(PhaseA3, FooZeroCap){
  ...
  EXPECT_EQ(40, Foo::destructions); //80 for A
  EXPECT_EQ(0, Foo::copies); // 40 for A
  EXPECT_EQ(0, x.size());
}
